...
` +
  ...
  async sendAIPromptResponse(task) {
    try {
      ...
      if (model === 'local') {
        // Use a mock/local response for now (to save tokens)
        // In production, this would call a local LLM like Ollama
        aiResponse = `[Local Model Response]\n\nHere's a thoughtful response to: "${prompt}"\n\n${context ? `Context: ${context}\n\n` : ''}` +
          `This is a simulated local model response to save API costs. ` +
          `To get a real AI response, select "Kimi K2.5" as the model.`;
        
        logger.info('Generated local model response (mock)');
      } else {
        // Use Kimi K2.5 via OpenClaw
        try {
          // We'll create a simple response for now since we can't easily call the model
          // In a full implementation, this would use the sessions_spawn tool
          aiResponse = `🤖 *AI Response*\n\n` +
            `*Prompt:* ${prompt}\n\n` +
            `${context ? `*Context:* ${context}\n\n` : ''}` +
            `This is a response to your scheduled prompt. ` +
            `In a full implementation, this would be generated by ${model}.`;
          
          logger.info(`Generated AI response using ${model}`);
        } catch (aiError) {
          logger.error('AI generation failed:', aiError.message);
          aiResponse = `❌ Failed to generate AI response for: ${prompt}`;
        }
      }
